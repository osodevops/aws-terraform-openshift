resource "aws_instance" "master_node" {
  ...
  root_block_device {
    ...
    volume_type           = "gp2"
    volume_size           = var.master_node_root_disk_size
  }
  ...
  ebs_block_device {
    device_name           = "/dev/sdf"
    volume_size           = var.master_node_data_volume_size
    volume_type           = "gp2"
    delete_on_termination = true
    encrypted             = false
  }
  ...
  depends_on = [
    aws_s3_bucket_object.master_post_install_script
  ]

  tags = merge(local.common_tags,
         map("Name", "${upper(var.environment)}-MASTER-NODE-${count.index}"))
}